...
import SidebatChatList from "@/components/SidebarChatList";
  ...
  return (
    <main className="w-full flex h-screen">
      <div className="flex h-full w-full max-w-xs grow flex-col  gap-y-5 overflow-y-auto border-r border-gray-200 bg-white px-6">
        ...
        {friends.length > 0 ? <div className="text-xs font-semibold leading-6 text-gray-400">Your chat</div> : null}

        <nav className="flex flex-1 flex-col">
          <ul role="list" className="flex flex-1 flex-col gap-y-7">
            <li>
              <SidebatChatList friends={friends} sessionId={session.user.id} />
            </li>
            <li>
              ...
              <ul role="list" className="-mx-2 mt-2 space-y-1">
                {sidebarOptions.map((option) => {
                  ...
                  return (
                    <li key={option.id}>
                      <Link href={option.href} className="text-gray-700 hover:text-indigo-600 hover:bg-gray-100 group flex gap-3 rounded-md p-2 text-sm leading-6 font-semibold">
